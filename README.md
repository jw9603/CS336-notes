# CS336-notes

Stanford **CS336: [Language Modeling from Scratch](https://cs336.stanford.edu/)** 강의를 들으며 정리한 개인 학습 노트입니다.

## About the Course

- **Course**: CS336 — Language Modeling from Scratch (Stanford University)
- **Topic**: LLM을 바닥부터 직접 구현하며 tokenization, transformer architecture, training, scaling, efficiency, alignment 등을 다룹니다.

## Notes

| Lecture | Title | Link |
|---|---|---|
| 01 | Overview and Tokenization | [lecture_notes/lecture01.md](lecture_notes/lecture01.md) |
| 02 | Resource Accounting (Systems) | [lecture_notes/lecture02.md](lecture_notes/lecture02.md) |
| 03 | LM Architecture and Hyperparameters | [lecture_notes/lecture03.md](lecture_notes/lecture03.md) |
| 04 | Attention Alternatives and Mixture of Experts | [lecture_notes/lecture04.md](lecture_notes/lecture04.md) |
| 05 | GPUs, CUDA, Performance, FlashAttention | [lecture_notes/lecture05.md](lecture_notes/lecture05.md) |
| 09 | Scaling Laws — Basics | [lecture_notes/lecture09.md](lecture_notes/lecture09.md) |

> 강의를 따라가며 노트는 계속 추가/업데이트됩니다.

## Disclaimer

본 repo의 내용은 개인 학습 목적의 정리이며, 강의 슬라이드 및 영상의 저작권은 Stanford University 및 강의자에게 있습니다.
